...
@section('topbar-actions')
    <a href="{{ route('admin.about-section3.edit') }}" class="cms-btn">Pengaturan Heading</a>
            ...
            <tbody>
                @forelse($milestones as $milestone)
                    <tr>
                        ...
                        <td><span
                                class="cms-badge {{ $milestone->is_active ? 'cms-badge-green' : 'cms-badge-gray' }}">{{ $milestone->is_active ? 'Aktif' : 'Non-aktif' }}</span>
                        </td>
                        <td class="cms-actions">
                            <a href="{{ route('admin.about-milestones.edit', $milestone) }}" class="cms-btn cms-btn-sm">Edit</a>
                            <form method="POST" action="{{ route('admin.about-milestones.destroy', $milestone) }}"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="cms-empty">Belum ada data.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
