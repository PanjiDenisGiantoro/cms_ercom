...
@section('breadcrumb', 'Settings / About / Section 3 / Edit')
...
@section('content')
<form id="milestone-form" method="POST" action="{{ route('admin.about-milestones.update', $milestone) }}">
    @csrf @method('PUT')
    <div class="cms-card">
        <div class="cms-form-row">
            <div class="cms-field">
                <label class="cms-label">Tahun <span class="cms-required">*</span></label>
                <input type="text" name="year" value="{{ old('year', $milestone->year) }}" class="cms-input @error('year') is-error @enderror" placeholder="2018">
                @error('year')<span class="cms-error">{{ $message }}</span>@enderror
            </div>
            <div class="cms-field">
                <label class="cms-label">Order</label>
                <input type="number" name="order" value="{{ old('order', $milestone->order) }}" class="cms-input" min="0">
            </div>
        </div>
        <div class="cms-field" style="margin-top:14px">
            <label class="cms-label">Headline <span class="cms-required">*</span></label>
            <input type="text" name="headline" value="{{ old('headline', $milestone->headline) }}" class="cms-input @error('headline') is-error @enderror">
            @error('headline')<span class="cms-error">{{ $message }}</span>@enderror
        </div>
        <div class="cms-field" style="margin-top:14px">
            <label class="cms-label">Deskripsi Headline</label>
            <textarea name="headline_description" rows="3" class="cms-input ckeditor">{{ old('headline_description', $milestone->headline_description) }}</textarea>
