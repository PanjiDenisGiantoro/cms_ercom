@extends('layouts.admin')
@section('title', 'Edit Kategori Layanan')
@section('page-title', 'Edit Kategori Layanan')
@section('breadcrumb', 'Content / Services / Edit')

@section('topbar-actions')
    <a href="{{ route('admin.services.index') }}" class="cms-btn">Kembali</a>
    <button form="form" type="submit" class="cms-btn cms-btn-primary">Simpan</button>
@endsection

@section('content')
<form id="form" method="POST" action="{{ route('admin.services.update', $service) }}" enctype="multipart/form-data">
    @csrf @method('PUT')
    <div class="cms-card">
        <div class="cms-form-row">
            <div class="cms-field">
                <label class="cms-label">Nama Kategori <span class="cms-required">*</span></label>
                <input type="text" name="name" value="{{ old('name', $service->name) }}" class="cms-input">
            </div>
            <div class="cms-field">
                <label class="cms-label">Order</label>
                <input type="number" name="order" value="{{ old('order', $service->order) }}" class="cms-input" min="0">
            </div>
        </div>
        <div class="cms-field" style="margin-top:14px">
            <label class="cms-label">Cover Image</label>
            @if($service->cover_image)
                <img src="{{ Storage::url($service->cover_image) }}" class="cms-img-preview" alt="Cover">
            @endif
            <input type="file" name="cover_image" class="cms-input" accept="image/*">
        </div>
        <div class="cms-field" style="margin-top:14px">
            <label class="cms-toggle-label">
                <input type="hidden" name="is_active" value="0">
                <input type="checkbox" name="is_active" value="1" {{ $service->is_active ? 'checked' : '' }}>
                <span>Aktif</span>
            </label>
        </div>
    </div>
</form>
@endsection
